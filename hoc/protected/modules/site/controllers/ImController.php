...
ON dialogs.userid = users.id WHERE lastMessage IS NOT NULL')->bindParam(':id', $myId, PDO::PARAM_INT)->queryAll();
    ...
    public function actionMobileChat($user_id)
    {
        ...
        Chat::model()->updateAll(['is_read' => 1], 'user_from = :from AND user_to = :myid AND is_read = 0', [':from' => $user_id, ':myid' => $this->user->id]);
        ...
        $this->layout = 'mobile_feed';
        $this->render('mobileChat', compact('messages', 'user_id', 'user'));
    }
