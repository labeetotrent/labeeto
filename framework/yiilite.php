...
EOD;
    ...
    public static function activeHtmlArea($model,$attribute,$htmlOptions=array())
	{
		self::resolveNameID($model,$attribute,$htmlOptions);
        $htmlOptions['id'] = 'wysiwyg';
		self::clientChange('change',$htmlOptions);
		if($model->hasErrors($attribute))
			self::addErrorCss($htmlOptions);
		$text=self::resolveValue($model,$attribute);
		return '<div class="block-fluid" id="wysiwyg_container">'.self::tag('textarea',$htmlOptions,isset($htmlOptions['encode']) && !$htmlOptions['encode'] ? $text : self::encode($text)).'</div>';
	}
        ...
        foreach($model->getValidators($attribute) as $validator){
            ...
            if ($validator instanceof CStringValidator){
                if ($validator->max!==null){
                    $validates[] = 'maxSize['.$validator->max.']';
                }
                if ($validator->min!==null){
                    $validates[] = 'minSize['.$validator->min.']';
                }
            }
            if ($validator instanceof CEmailValidator){ 
                $validates[] = 'custom[email]';
            }
            
            if ($validator instanceof CNumberValidator){
                $htmlOptions['class'] .= ' numberic_only';
            }          
		}
        if (count($validates)){
            $htmlOptions['class'] .= ' validate['.implode(',', $validates).']';
        }
