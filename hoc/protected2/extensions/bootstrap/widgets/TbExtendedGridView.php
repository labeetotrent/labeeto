...
class TbExtendedGridView extends TbGridView {
	...
	public function renderAddNewBtn(){
		$link = isset($this->addNew['url'])?$this->addNew['url']:'#';
		$label = isset($this->addNew['label'])?$this->addNew['label']:'Create';
		$class = isset($this->addNew['htmlOptions']['class'])?$this->addNew['htmlOptions']['class']:array();
		$class = implode($class, ' ');
		$align = isset($this->addNew['align'])?$this->addNew['align']:'pull-left';

		$icon = isset($this->addNew['icon'])?$this->addNew['icon']:'fa-plus-square';
		echo "<a class='$align btn btn-success $class' href='{$link}'><i class='fa $icon'></i> $label</a>";
	}
	/**
	 *### .renderKeys()
	 *
