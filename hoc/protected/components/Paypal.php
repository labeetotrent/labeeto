    ...
    */
    ...
    public function __construct(){
        if(isset(Yii::app()->session['shop_buy'])){
            if(Yii::app()->session['shop_buy'] ==0){ // payment of Tosello
                $settings = PaymentMethods::getSettings(PaymentMethods::PAYPAL);
                foreach ($settings as $key=>$value)
                    $this->$key = $value;
            } else {
                // payment of shop owner.
                $settings = MemberShop::model()->getSettings(Yii::app()->session['shop_buy']);
                if($settings){
                    foreach ($settings as $key=>$value)
                        $this->$key = $value;
                }
    ...
    public function SetExpressCheckout($paymentInfo=array()){
        ...
        //quantity
        if(isset($paymentInfo['Order']['quantity'])){
        ...
        $nvpstr='&ALLOWNOTE=1&AMT='.$amount.'&PAYMENTACTION='.$paymentType.'&CURRENCYCODE='.$currencyCode.'&RETURNURL='.$returnURL.
                '&CANCELURL='.$cancelURL.'&DESC='.$desc.'&QTY='.$quantity
                .'&HDRIMG='.Yii::app()->createAbsoluteUrl('/themes/default/img/logo.png');
        /*
        foreach ($paymentInfo['Items'] as $key=>$value){
            $nvpstr .= "&$key=$value";
        }
        */
