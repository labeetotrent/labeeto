...
class BlogController extends AdminBaseController {
     ...
     public function actionCreate()
	{
		...
		if(isset($_POST['Blog']))
		{
            ...
            $model->content      =   $_POST['Blog']['content'];
            $model->description  =   $_POST['Blog']['description'];
			...
			if($model->save()){
                if(!empty($uploadedFile)) {
			         $uploadedFile->saveAs(Yii::app()->basePath.'/../uploads/blog/'.$fileName);
                }
    ...
    public function actionUpdate($id)
	{
		...
		if(isset($_POST['Blog']))
		{
			...
			if($model->save()){
			    if(!empty($uploadedFile)) { 
                    $uploadedFile->saveAs(Yii::app()->basePath.'/../uploads/blog/'.$model->image);
                }
				$this->redirect(array('view','id'=>$model->id));
            }
		}
