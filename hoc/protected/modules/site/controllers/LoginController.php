...
class LoginController extends SiteBaseController
{
	...
	{	
		...
		if(isset($_POST['LostPasswordForm']))
	    {
			...
			{					
                ...
                $subject = Yii::t('global', 'Welcome to ' . Yii::app()->name);
                Utils::sendMail(Yii::app()->params['emailout'], $member->email, $subject, $message);
	...
	public function actionreset()
	{
        ...
        $template = EmailTemplates::getAlias('reset-password');
        $email = new EmailTemplates;
		$message = Yii::t('login', $email->email_content,
									array( '{username}' => $member->username, '{password}' => $password ));
									
		$message .= Yii::t('global', '<br /><br />----------------<br />Regards,<br />The {team} Team.<br />', array('{team}'=>Yii::app()->name));							
        
        Utils::sendMail(Yii::app()->params['emailout'], $member->email,  $email->email_subject, $message);
